...
- **2026-08-19 — Phase 7b merged (`#1`, `6923878`).** `infra/production-reference/` and the small `.gitignore`/README doc fixes landed on `main` via PR review, not a direct push.

- **2026-08-19 — Live countdown for upcoming sales, merged (`#2`, `c31f5e9`).** `StatusBanner` ticks a local "Starts in M:SS" countdown while the sale is `upcoming` and triggers an immediate status refetch the instant it hits zero, instead of waiting up to 5s for the next poll. Purely a UX affordance — the countdown runs off the browser clock, but `SaleService.isActive()` still checks server time on every request, so client-side drift can't affect purchase eligibility. Verified live against a running server with a short seeded window: watched it tick down and confirmed the automatic flip to `ACTIVE` with no manual reload.

- **2026-08-19 — One-command demo boot, merged (`#3`, `0cd22e1`).** Added `npm run demo` at the repo root: `docker compose up -d --wait` → seed an `upcoming` sale starting in 45s → `npm run start:all`, so presenting the app to someone is one command instead of Docker + two `npm install`s + a manual seed. Extended `server/scripts/seed-sale.ts` with `--in=<seconds>` to control how far out an `--state=upcoming` sale starts (default 3600s, unchanged) — the demo script uses a short window specifically so the live countdown from the previous entry is the first thing a viewer sees. Verified end-to-end from a cold `docker compose down`.
